...
const BGPattern = class BGPattern {

  constructor(canvas, patternURL) {
    const that = this;
    this.canvas = canvas;

    this.patternGradientColor1 = '#233248';
    this.patternGradientColor2 = '#263E42';
    this.patternGradientColor3 = '#383E36';

    this.imageLoaded = false;
    this.patternImage = new Image();
    this.patternImage.src = patternURL;
    ...
    window.addEventListener('resize', function () {
      if(that.imageLoaded) {
        // Draw function uses pattern image so it has to be loaded
        that.draw();
      }
    });
  }
  ...
  draw() {
    ...
    let gradient = context.createLinearGradient(0, 0, this.canvasWidth, 0);
    gradient.addColorStop(0, this.patternGradientColor1);
    gradient.addColorStop(0.5, this.patternGradientColor2);
    gradient.addColorStop(1, this.patternGradientColor3);
    ...
    context.fillStyle = context.createPattern(this.patternImage, 'repeat');
    context.fillRect(0, 0, this.canvasWidth, this.canvasHeight);
  }
};

new AnimatedGrid(document.querySelector('#animated-grid'));
new BGPattern(document.querySelector('#bg-pattern'), 'pattern.png');
